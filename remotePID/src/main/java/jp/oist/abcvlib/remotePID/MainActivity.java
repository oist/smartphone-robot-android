package jp.oist.abcvlib.remotePID;
...
public class MainActivity extends AbcvlibActivity {
    ...
    public class PID implements Runnable{
        ...
        int outputL; //  u(t) of wikipedia
        int outputR; //  u(t) of wikipedia
...
//                    Log.i("theta", "theta was updated in " + (updateTimeStep / 1000000) + " ms");
                ...
                try {
                    outputL = (int) Double.parseDouble(controls.get("wheelSpeedL").toString());
                    outputR = (int) Double.parseDouble(controls.get("wheelSpeedR").toString());
...
//                    Log.i("timers", "PIDTimer Averages = " + Arrays.toString(PIDTimeSteps));
        ...
        public void run(){
            ...
            while (true){
                ...
                while(!socketClient.ready) {
                    try {
                        Thread.sleep(100);
                        Log.i("abcvlib", "socketClient not ready");
    //                    socketClient.connect();
                ...
                if(socketClient.writePermission){
                    pythonControlTimer[0] = System.nanoTime();
                    writeAndroidData();
                    System.out.println(inputs);
                }
                else if (socketClient.readPermission){
